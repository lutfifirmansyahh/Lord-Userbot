...
# =================== CONSTANT ===================
PP_TOO_SMOL = "`Gambar Kekecilan pepek`"
PP_ERROR = "`Gagal Memproses Gambar`"
NO_ADMIN = "`Gabisa pepek lu bukan admin:)`"
NO_PERM = "`Lu ga Punya Izin!`"
NO_SQL = "`Berjalan Pada Mode Non-SQL`"

CHAT_PP_CHANGED = "`Berhasil Mengubah Profil Grup`"
CHAT_PP_ERROR = (
    "`Ada Masalah Dengan Memperbarui Foto,`"
    "`Mungkin Karna Lu Bukan Admin,`"
...
async def set_group_photo(gpic):
    if not gpic.is_group:
        await gpic.edit("`BOSHT Mohon Lakukan Perintah Ini Di Grup.`")
        return
...
async def promote(promt):
    ...
    await promt.edit("`Promosikan Temenlu Sebagai Admin... Tunggu sebentar pepek`")
    ...
    try:
        ...
        await promt.edit("`Berhasil Mempromosikan Temen lu jadi Admin!`")
        await sleep(5)
        await promt.delete()
...
async def ban(bon):
    ...
    if reason:
        await bon.edit(
            f"`PENGGUNA:` [{user.first_name}](tg://user?id={user.id})\n`ID:` `{str(user.id)}` MAMPUS LU GUE BAN!!\n`Alasan:` {reason}"
        )
    else:
        await bon.edit(
            f"`PENGGUNA:` [{user.first_name}](tg://user?id={user.id})\n`ID:` `{str(user.id)}` MAMPUS MAKAN TUH PEPEK!"
        )
...
async def nothanos(unbon):
    ...
    await unbon.edit("`Hehehe Sedang Melakukan Unban...`")
...
async def spider(spdr):
    ...
    if user.id == self_user.id:
        return await spdr.edit(
            "`Lu tolol ya?, Gabisa ngebisuin diri sendiri pepek...\n(ヘ･_･)ヘ┳━┳`"
        )

    # If everything goes well, do announcing and mute
    await spdr.edit("`Telah Dibisukan!`")
    if mute(spdr.chat_id, user.id) is False:
        return await spdr.edit("`Error! Pengguna udah bisu BOSHT.`")
    else:
        try:
            ...
            if reason:
                ...
                await spdr.edit("`Kesian bisu haha!`")
...
async def unmoot(unmot):
    ...
    if unmute(unmot.chat_id, user.id) is False:
        return await unmot.edit("`Udah ga bisu kan Pepek.`")
    else:

        try:
            await unmot.client(EditBannedRequest(unmot.chat_id, user.id, UNBAN_RIGHTS))
            await unmot.edit("```Melakukan Unmute! udah ga bisu lagi```")
            await sleep(3)
            await unmot.delete()
        except UserIdInvalidError:
...
async def rm_deletedacc(show):
    ...
    del_u = 0
    del_status = "`Tidak Menemukan Akun Jurig, Grup aman dan bersih`"

    if con != "clean":
        await show.edit("`Mencari Akun Jurig/Terhapus/Blegug...`")
        ...
        if del_u > 0:
            del_status = (
                f"`Menemukan` **{del_u}** `Akun Jurig/Terhapus/Zombie Dalam Grup Ini,"
    ...
    # Well
    if not admin and not creator:
        return await show.edit("`Lu Bukan Admin Disini pepek!`")

    await show.edit("`Menghapus Akun Jurig...\nMohon Menunggu... Sedang Dalam Proses`")
    del_u = 0
    del_a = 0
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                return await show.edit("`Lu gapunya Izin Banned Dalam Grup Ini pepek`")
    ...
    if del_u > 0:
        del_status = f"`Membersihkan` **{del_u}** `Akun Jurig`"

    if del_a > 0:
        del_status = (
            f"Membersihkan **{del_u}** Akun Jurig "
            f"\n**{del_a}** `Admin Akun Jurig Ga Bisa Dihapus.`"
        )
...
async def get_admin(show):
    ...
    mentions = f"<b> Daftar Admin Pepek {title}:</b> \n"
    try:
...
async def pin(msg):
    ...
    if not to_pin:
        return await msg.edit("`Lu pepek,kalo mau pin bales ke pesan yang mau di pin.`")
...
async def kick(usr):
    ...
    await usr.edit("`LOADING Lagi nge kick si pepek....`")
    ...
    if reason:
        await usr.edit(
            f"[{user.first_name}](tg://user?id={user.id}) **Sikontol berhasil di Kick**\n**Alasan:** `{reason}`"
        )
    else:
        await usr.edit(f"[{user.first_name}](tg://user?id={user.id}) **Si Pepek Dikick Dari Grup**")
        await sleep(5)
        await usr.delete()
...
async def get_users(show):
    ...
    try:
        ...
        await show.edit("Pepek, Grup Ini Terlalu Besar Mengunggah Daftar Pengguna Sebagai File.")
...
async def get_user_from_event(event):
    ...
    if event.reply_to_msg_id and len(args) != 2:
        ...
        if not user:
            return await event.edit("`Heh Pepek! Ketik Username Atau Balas Ke Pengguna!`")
...
async def get_usersdel(show):
    ...
    try:
        ...
        await show.edit(
            "Bosht, Grup Ini Terlalu Besar, Mengunggah Daftar Akun Terhapus Sebagai File."
        )
        file = open("daftarpengguna.txt", "w+")
        file.write(mentions)
        file.close()
...
async def get_userdel_from_event(event):
    ...
    if event.reply_to_msg_id and len(args) != 2:
        ...
        if not user:
            return await event.edit("`Heh Pepek! Ketik username Atau Balas Ke Pesan Pengguna!`")
...
async def get_bots(show):
    ...
    try:
        ...
        await show.edit("BOSHT, Terlalu Banyak Bot Di Grup Ini, Mengunggah Daftar Bot Sebagai File.")
...
CMD_HELP.update(
    {
        ...
        "\n\n>`.zombies`"
        "\nUsage: Untuk mencari akun jurig dalam grup. "
        "Gunakan `.zombies clean` untuk menghapus Akun Jurig dari grup."
        "\n\n>`.all`"
        ...
        "\nUsage: Mendapatkan daftar pengguna dalam grup."
